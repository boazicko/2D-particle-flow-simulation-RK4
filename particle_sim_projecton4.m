...
%stmp=stmp(length(stmp)-7:length(stmp));%no date
stmp = strrep(stmp, ':', '_');%windows naming characters
txt='mooving_arbol_';%movie name
name=[txt stmp];
name=char(name);
...
R=3.98;%for viscircle
[xPositions, yPositions] = meshgrid(-boundsx:parspace:+boundsx,-boundsy:parspace:boundsy+upplus);%starting particles
N=numel(xPositions);
...
%update the location of particles with starting locations
for i = 1:N
    particles(i).x = xPositions(i);
    particles(i).y = yPositions(i);

end
%possible figure positions
...
wheel=linspace(0,2*pi(),9);%thetas for points on wheel
%% velocity function
eps=10^-1;
%rmp func, from value a to value b starting at t1 up to  t2, t required to
%return value
...
Q=100;
boost=100;
rm=0.6;% random particle creation at a distance of up to rm from the makor.
tz1=15;
tz2=18;
d1=7;
d2=0.8;
D=25;
varbol=2*D/30;
Xm=@(t) rmp(t,D-d2,-D-d2,0,30);%makor location
Qm=@(t) rmp(t,0,Q,0,0.1);%makor strength
Vrm=@(x,y,t) Qm(t)/2/pi()/sqrt(x^2+y^2+eps^2);% V radial
Vm=@(x,y,t) [Vrm(x,y,t)*cos(atan2(y,x))...
             Vrm(x,y,t)*sin(atan2(y,x))]; % V cartizian

%bor % same same but different
rb=0.8;
Xb=@(t) rmp(t,D+d2,-D+d2,0,30);
Qb=@(t) Qm(t);
...
%% RK4 update
   ...
   if dis>parspace
       dis=0;
       disturb=-disturb;%for seperation of flow around the cone.
...
%     color=@(t) [1 rmp(t,1,0.01,22.5,25) rmp(t,1,0.01,22.5,25)];
    color=@(t) [1 0 0];
    if t>0
    viscircles([(Xm(t)+Xb(t))/2,0],R,'Color',color(t),'LineWidth',5);
    wheel=wheel+omega(t)/fps;
    plot(R*cos(wheel)+(Xm(t)+Xb(t))/2,R*sin(wheel),'.','LineWidth',3,'MarkerSize',30,'Color',color(t))
    end
    plot(0,0,'r.')
    %txt
